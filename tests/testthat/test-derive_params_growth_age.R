...
test_that("derive_params_growth_age Test 5: Extreme BMI value derivation works", {
  ...
  tmpexpected <- vs_sds %>%
    ...
    mutate(
      PCTL = if_else(tmpPCTL / 100 > 0.95, 90 + 10 * pnorm((VSSTRESN - P95) / Sigma), tmpPCTL),
      SDS = if_else(tmpPCTL / 100 > 0.95, if_else(tmpPCTL / 100 == 1, 8.21, qnorm(PCTL / 100)), tmpSDS) # nolint
    ) %>%
