...
- [Derive Additional Variables for Anthropometric
  indicators](#derive_vars)
  - [Importance of Age Units using
    `derive_params_growth_age()`](#age-unit-handling)  
- [Derive Additional Parameters for Anthropometric
  indicators](#derive_params)
...
#### Reference Files for HEIGHT

WHO for children \<2 yrs old (\< 730.5 days), and CDC for children \>=2
yrs old (\>= 730.5 days)
...
Here is how the first records of the WHO metadata for HEIGHT now look:
...
##### Age Unit Handling and Standardization

##### Understanding Age Unit Requirements
...
function requires that the age unit in your input data **exactly
matches** the age unit in your metadata. The function does **NOT**
automatically convert between age units.

**Important**: If your data has ages in days but your metadata is in
months (or vice versa), records will not be matched and warnings will be
issued. These unmatched records will be excluded from the output.

##### Common Age Unit Conversion Factors

When standardizing age units, use these conversion factors:

- 1 week = 7 days
- 1 month = 30.4375 days
- 1 year = 365.25 days

##### Method: Manual Age Unit Conversion

If you prefer to manually convert age units in your dataset to match
your metadata, you can use conditional logic. Based on the `advs`
dataset created earlier with the `AAGECUR` (current age) and `AAGECURU`
(current age unit) variables:
...
#### Use the standardized age variables
