...
WHO for children <2 yrs old (< 730.5 days), and CDC for children >=2 yrs old (>= 730.5 days)
...
#### Age Unit Handling and Standardization {#age-unit-handling}

#### Understanding Age Unit Requirements

The `derive_params_growth_age()` function requires that the age unit in your input data **exactly matches** the age unit in your metadata.
The function does **NOT** automatically convert between age units.

**Important**: If your data has ages in days but your metadata is in months (or vice versa),
records will not be matched and warnings will be issued. These unmatched records will be
...
If you prefer to manually convert age units in your dataset to match your metadata,
you can use conditional logic. Based on the `advs` dataset created earlier with the
`AAGECUR` (current age) and `AAGECURU` (current age unit) variables:
...
|------|-------------------------------------------------------------|
| [`ADVS`](templates.html#advs) | `admiral::use_ad_template("ADVS", package = "admiralpeds")` |
