...
Note that the head circumference parameter metadata comes only from the WHO standards,
as CDC provides no equivalent for children \> 2 years of age.

### Reference Files for Weight by Length/Height

WHO provides additional standards for weight-for-length (recumbent length), 
...
For example when you look at the "by Age" charts, WHO provides this by day,
...
supplied to the above function via the via the `meta_criteria` argument.
