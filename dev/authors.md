# Authors and Citation

## Authors

- **Fanny Gautier**. Author, maintainer.
  [](https://orcid.org/0009-0004-3581-0131)

- **Ross Farrugia**. Author.

- **Zelos Zhu**. Author.

- **Sukalpo Saha**. Author.

- **Lina Patil**. Author.

- **Samia Kabi**. Author.

- **Laura Liao**. Contributor.

- **Remigiusz Kudlacz**. Contributor.

- **Pierre Wallet**. Contributor.

- **Amin Sherzad**. Contributor.

- **David Freedman**. Contributor.

- **Mahmoud Hamza**. Contributor.

- **[](https://www.cytel.com)[![Cytel
  logo](reference/figures/cytel_logo.png)](https://www.cytel.com/)**.
  Copyright holder, funder.

- **[](https://www.roche.com)[![Roche
  logo](reference/figures/roche_logo.png)](https://www.roche.com/)**.
  Copyright holder, funder.

- **[](https://www.pfizer.com)[![Pfizer
  logo](reference/figures/pfizer_logo.png)](https://www.pfizer.com/)**.
  Copyright holder, funder.

- **[](https://www.atorusresearch.com)[![Atorus
  logo](reference/figures/atorus_logo.png)](https://www.atorusresearch.com/)**.
  Copyright holder, funder.

## Citation

Source:
[`DESCRIPTION`](https://github.com/pharmaverse/admiralpeds/blob/main/DESCRIPTION)

Gautier F, Farrugia R, Zhu Z, Saha S, Patil L, Kabi S (2026).
*admiralpeds: Pediatrics Extension Package for ADaM in 'R' Asset
Library*. R package version 0.3.0.9004,
<https://pharmaverse.github.io/admiralpeds/>.

    @Manual{,
      title = {admiralpeds: Pediatrics Extension Package for ADaM in 'R' Asset Library},
      author = {Fanny Gautier and Ross Farrugia and Zelos Zhu and Sukalpo Saha and Lina Patil and Samia Kabi},
      year = {2026},
      note = {R package version 0.3.0.9004},
      url = {https://pharmaverse.github.io/admiralpeds/},
    }
